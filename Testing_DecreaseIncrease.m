...
%         z_temp2 = 0;
        ...
        location = j;
        for iter = 3: j
            time_ktau_my = time_ktau(end-iter+1:end);
            AC_timeseries_ktau_my = AC_timeseries_ktau(end-iter+1:end);

            % Calculate Kendall-tau and determine whether to reject or retain null hypothesis
            [tau_my, z_my, p_my, H_my] = Modified_MannKendall_test(time_ktau_my, AC_timeseries_ktau_my, significance_value_tau, significance_value_ac, gpu_shift_critical_size, print_bool);
...
%         figure(11);
%         clf;
%         plot(EWS_details.time_window_ends(selection), EWS_details.AC_timeseries(selection) );
%         hold on
%         plot(EWS_details.time_window_ends(location), EWS_details.AC_timeseries(location), 'Marker', 'o', 'MarkerFaceColor', 'blue', 'MarkerSize', 10);
%         pause(0.5);
...
pred_frac_calc_time = toc(pred_frac_calc_start);
fprintf("pred_frac_calc_time = %f\n", pred_frac_calc_time);
